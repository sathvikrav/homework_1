...
T scan_inplace(T *A, size_t n, const F& f, T id) {
  ...
  T total = scan_up(A, n, L, n - 1, f, id);
  free(L);
  return total;
}

template <class T, class F>
size_t scan_up(T *A, size_t n, T *L, size_t k, const F& f, T id) {
  // if (n == 1) {
  //   return A[0];
  // } 
  if (n <= 10000) {
    ...
    size_t m = n / 2;
    T l, r;
    auto f1 = [&]() { l = scan_up(A, m, L, m - 1, f, id); };
    auto f2 = [&]() { r = scan_up(A + m, n - m, L + m, k - m, f, id); };
    par_do(f1, f2);
    L[m - 1] = l;
    return f(l, r);
  }
...
//     auto f2 = [&]() { scan_down(A + m, n - m, L + m, k - m, f, f(id, L[m - 1])); };
//     par_do(f1, f2);
//     return;
//   }
// }
